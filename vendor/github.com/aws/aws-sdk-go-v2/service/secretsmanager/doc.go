...
// information about the AWS SDKs, including downloading and installing them, see
// Tools for Amazon Web Services (http://aws.amazon.com/tools/). We recommend you
// use the AWS SDKs to make programmatic API calls to Secrets Manager. However, you
// also can use the Secrets Manager HTTP Query API to make direct calls to the
// Secrets Manager web service. To learn more about the Secrets Manager HTTP Query
// API, see Making Query Requests
// (https://docs.aws.amazon.com/secretsmanager/latest/userguide/query-requests.html)
// in the AWS Secrets Manager User Guide. Secrets Manager API supports GET and POST
// requests for all actions, and doesn't require you to use GET for some actions
...
// Support and Feedback for AWS Secrets Manager We welcome your feedback. Send your
// comments to awssecretsmanager-feedback@amazon.com
// (mailto:awssecretsmanager-feedback@amazon.com), or post your feedback and
// questions in the AWS Secrets Manager Discussion Forum
...
// that AWS Secrets Manager expects as your request parameters and the service
// returns as a response to HTTP query requests contain single, long strings
// without line breaks or white space formatting. The JSON shown in the examples
// displays the code formatted with both line breaks and white space to improve
// readability. When example input parameters can also cause long strings extending
// beyond the screen, you can insert line breaks to enhance readability. You should
// always submit the input as a single JSON text string. Logging API Requests AWS
...
// see Logging AWS Secrets Manager Events with AWS CloudTrail
...
// enabling it and find your log files, see the AWS CloudTrail User Guide
